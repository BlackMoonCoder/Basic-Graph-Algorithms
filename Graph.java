...
/***------------------  BEGIN AUGEMENTED METHOD ----------------***/
   ...
   public boolean isEdge(E fromKey, E toKey)
   {
      //implement this function
		...
		Vertex fromtmp = first;
		while (fromtmp != null && cmp.compare(fromKey, fromtmp.data) != 0) {
			fromtmp = fromtmp.pNextVertex;
		}
		Vertex totmp = first;
		while (totmp != null && cmp.compare(toKey, totmp.data) != 0) {
			totmp = totmp.pNextVertex;
		}
		
		Edge tmpE = fromtmp.pEdge;
		while (tmpE != null && tmpE.destination != totmp) {
			tmpE = tmpE.pNextEdge;
		}
		if (tmpE == null) {
			return false;
